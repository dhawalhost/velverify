...
import { login, getBranding, completeMfaLogin, lookupUser, beginLogin, finishLogin, getSetupStatus, getUserRoles, api, getUserProfile, getPublicSSOProviders } from '../api';
...
const Login: React.FC = () => {
    ...
    const [tenantID, setTenantID] = useState(() => {
        return pathTenantID || searchParams.get('tenant') || '';
    });
    ...
    const fetchBranding = async (tid: string) => {
        ...
        try {
            ...
            if (config.css_override) {
                // Remove any existing dynamic style tags for this tenant to avoid duplicates
                const existingStyle = document.getElementById('tenant-css-override');
                if (existingStyle) {
                    existingStyle.remove();
                }
                const style = document.createElement('style');
                style.id = 'tenant-css-override';
    ...
    const fetchSSOProviders = async (tid: string) => {
        if (!tid) return;
        try {
            const providers = await getPublicSSOProviders(tid);
    ...
    const handleSocialLogin = (providerName: string) => {
        ...
        const authUrl = `${import.meta.env.VITE_AUTH_URL}/sso/login/${providerName}?tenant_id=${tenantID}`;
    ...
    useEffect(() => {
        if (tenantID) {
            ...
            fetchSSOProviders(tenantID);
            localStorage.setItem('tenantID', tenantID);
            
            const searchParams = new URLSearchParams(location.search);
            const redirectUri = searchParams.get('redirect_uri');
            if (redirectUri) {
                localStorage.setItem('login_redirect_uri', redirectUri);
            }
        }
    ...
    return (
        <div 
            className="min-h-screen flex flex-col lg:flex-row bg-surface text-on-surface selection:bg-primary/20"
                ...
                <div className="relative z-10">
                    <div className="flex items-center gap-3 group">
                        <div className="w-10 h-10 flex items-center justify-center">
                            <img src={branding.logo_url || "/wardseal.svg"} alt="WardSeal" className="w-full h-full object-contain" />
                        </div>
                        <span className="text-2xl font-bold tracking-tight text-white group-hover:opacity-80 transition-opacity">
                            {branding.tenant_name || "WardSeal"}
                        </span>
                    </div>
                </div>
            ...
            <div className="w-full lg:w-1/2 flex flex-col justify-center px-8 sm:px-12 lg:px-24 xl:px-32 py-12">
                <div className="w-full max-w-md mx-auto">
                    ...
                    <div className="lg:hidden mb-12 flex justify-center">
                        <img src={branding.logo_url || "/wardseal.svg"} alt="WardSeal" className="w-12 h-12" />
                    </div>

                    <div className="mb-10 text-center lg:text-left">
                        <h2 className="text-3xl font-bold tracking-tight text-on-surface mb-2">
                            {branding.tenant_name ? `Sign in to ${branding.tenant_name}` : "Welcome back"}
                        </h2>
                    ...
                    {step === 'identifier' ? (
                        <form onSubmit={handleIdentifierSubmit} className="space-y-6">
                            ...
                            <Button 
                                type="submit" 
                                className="w-full h-14 rounded-xl text-primary-foreground font-bold text-base shadow-lg shadow-primary/20 hover:shadow-primary/30 transition-all hover:-translate-y-0.5 active:translate-y-0 group" 
                                ...
                                <>
                                    <div className="relative py-4">
                                        ...
                                        <div className="relative flex justify-center text-[10px] font-bold uppercase tracking-widest">
                                            <span className="bg-card px-4 text-on-surface-variant/40 italic">Or sign in with</span>
                                        </div>
                                    </div>
                                    ...
                                    <div className="space-y-3">
                                        {ssoProviders.map(provider => (
                                            <Button
                                                ...
                                                type="button"
                                                variant="outline"
                                                className="w-full h-14 rounded-xl border-border bg-background/50 hover:bg-surface-container transition-all font-bold text-sm shadow-sm"
