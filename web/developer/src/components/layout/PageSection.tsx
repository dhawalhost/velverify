...
export const PageSection: React.FC<PageSectionProps> = ({
    ...
    return (
        <GlassCard
            className={cn(
                "border-none shadow-2xl shadow-on-surface/5 bg-card overflow-hidden rounded-xl",
                className
            )}
        >
            <GlassCardHeader className="py-4 px-6 border-b border-on-surface/5 bg-surface-container/5">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <GlassCardTitle className="text-lg font-bold tracking-tight text-on-surface uppercase">
                                {title}
                            </GlassCardTitle>
                            {description && (
                                <p className="text-on-surface-variant/60 font-medium text-[10px] mt-0.5 uppercase tracking-wide">
                                    {description}
                                </p>
                            )}
                        </div>
            ...
            <GlassCardContent className={cn(noPadding ? "p-0" : "p-6", contentClassName)}>
                {children}
            </GlassCardContent>
        </GlassCard>
    );
